...
class RegisterForm(FlaskForm):
    ...
    password = PasswordField(
        'password', validators=[Required(), Length(6, 24)])
...
class CourseForm(FlaskForm):
    ...
    description = TextAreaField(
        'course description', validators=[Required(), Length(20, 256)])
    image_url = StringField('cover image', validators=[Required(), URL()])
    author_id = IntegerField('author id', validators=[
                             Required(), NumberRange(min=1, message='invalid user id')])
    submit = SubmitField('submit')
...
class LiveForm(FlaskForm):
    name = StringField('live name', validators=[Required(), Length(1, 128)])
